    ...
    public class Start
    {
        public static async Task Generator(string basePath)
        {
            ...
            await new GeneratorHandler(basePath, builder =>
            {
                ...
                {
                    //指定输出的根目录
                    proto.Output = $"../golang/{project}.Protocol";

                    //指定 proto 文件存放目录
                    proto.ProtoDirectory = "protos";

                    //指定 Package 名称
                    proto.PackageNameFunc = meta => "assets";

                    //指定 C# 对应的命名空间
                    proto.CSharpNamespaceFunc = meta => $"{project}.GRpc.Protocol";

                    //指定 GoLang 对应的命名空间
                    proto.GoLangPackageFunc = meta => $"sailing.cn/protobuf/assets";

                    //获取注释
                    proto.PropertyDescriptionFunc = meta => meta?.Display ?? meta?.DisplayName ?? meta?.Description;

                    //处理 proto 中 service 名称
                    proto.ServiceNameFunc = meta => meta.Name.TrimStart('I').Replace("Service", "") + "Service";
                ...
                //{
                //    //指定输出的根目录
                //    server.Output = $"../golang/{project}.Server";
                ...
                //配置 go service 实现
                builder.SetGoServerConfig(server =>
                {
                    //指定输出的根目录
                    server.Output = $"../golang/{project}.Server";
                    ...
                    server.ServerNameFunc = meta => meta.Name.TrimStart('I').Replace("Service", "") + "Server";

                    server.ServerFileNameFunc = meta => meta.Name.TrimStart('I').Replace("Service", "").ToSnakeString() + ".grpc.go";

                    server.GoStorePackagePath = "service.cn/assets/store111";
                    server.GoStorePackage = "store";
                ...
                builder.SetGoStructConfig(dto =>
                {
                    dto.Output = $"../golang/{project}.dto";
                    dto.StructNameFunc = name => name.Replace("`1", "").Replace("`", "") + "Dto";
                    dto.PackageName = "assets_dto";
                });
                ...
                // 生成控制器配置
                builder.SetGoController(controller =>
                {
                    //指定输出的根目录
                    controller.Output = $"../golang/{project}.controller";
                    ...
                    //设置路由
                    controller.Route = $"media-assets";

                    // 指定一个函数名称，控制器返回结果包装，返回给前端，比如 指定 Success，则结果是 return Success(result)；默认是 return Ok(result)
                    controller.ReturnMethodName = "restful.Success";

                    // 命名空间、包名称
                    controller.NamespaceOrPackage = "assets";

                    // 依赖外部命名空间、引入包路径等
                    controller.Dependency = new List<string>()
                    {
            ...
            //        //指定 Package 名称
            //        proto.PackageNameFunc = meta => "aaaaaaaa";

            //        //指定 C# 对应的命名空间
            //        proto.CSharpNamespaceFunc = meta => $"{project}.GRpc.Protocol";

            //        //指定 GoLang 对应的命名空间
            //        proto.GoLangPackageFunc = meta => $"sailing.cn/protobuf/{project}";
